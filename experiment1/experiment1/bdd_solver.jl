...
isinside(c::Circle, p::Point) = (p.x - c.center.x)^2 + (p.y - c.center.y)^2 <= c.radius^2 + 1e-15

dist(a::Point,b::Point) = hypot(a.x-b.x, a.y-b.y)

function createarea(rect::Rectangle4, gridsize::Int)
    out = Vector{Rectangle4}()
    x0 = rect.v[1].x; y0 = rect.v[1].y
    x1 = rect.v[3].x; y1 = rect.v[3].y
    for i in 0:(gridsize-1)
        for j in 0:(gridsize-1)
            xa = x0 + (x1 - x0) * (i)/gridsize
            xb = x0 + (x1 - x0) * (i+1)/gridsize
            ya = y0 + (y1 - y0) * (j)/gridsize
            yb = y0 + (y1 - y0) * (j+1)/gridsize
            push!(out, Rectangle4((Point(xa,ya), Point(xa,yb), Point(xb,yb), Point(xb,ya))))
        end
    end
    return out
end
...
function bddsolver(M::MiniCUDD.Manager, vars::Dict{String,Int},
    ...
    pm = Progress(total; dt=0.3, desc="Refining (grid=$gridsize, maxlevel=$maxlevel)")
    last_total = total
    ...
    while !isempty(q)
        ...
        areas = createarea(rect, gridsize)
        for r in areas
...
function main(argv)
    ...
    sort!(circles, by = c->dist(c.center, Point(0.0,0.0)))
