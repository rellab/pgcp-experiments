...
const MyPoint = Point2{Float64}

@inline getx(p::MyPoint) = @inbounds p[1]
@inline gety(p::MyPoint) = @inbounds p[2]

dist(a::MyPoint,b::MyPoint) = norm(a - b)
...
@inline name(c::MyCircle) = c.name
@inline center(c::MyCircle) = c.c.center
@inline radius(c::MyCircle) = c.c.radius

isinside(c::MyCircle, p::MyPoint) = in(p, center(c))
