...
function createarea(rect::Rectangle4; gridsize::Int)
    ...
    @inbounds for j in 0:gridsize-1
        ya = y0 + dy * j
        yb = ya + dy
...
function load_config_from_json(path::String)
    ...
    for c in cfg["circles"]
        ...
        # push!(circles, Circle(name, Point(y,x), r))
    end
    return circles, gridsize, maxlevel, reliability
end

# draw
function plot_with_order(cs::Vector{Circle})
    # compute maximum and total distance
    totaldist = 0.0
    maxdist = 0.0
    prev = cs[1]
    for c in cs[2:end]
        d = dist(c.center, prev.center)
        totaldist += d
        if d > maxdist
            maxdist = d
        end
    ...
    xs = [getx(c.center) for c in cs]
    ys = [gety(c.center) for c in cs]
    ...
    plot(xs, ys,
        ...
        aspect_ratio = 1,
        legend = false,
        title = @sprintf("Total Distance: %.4f, Max Distance: %.4f", totaldist, maxdist)
    )
end
...
function sort_with_center(circles::Vector{Circle})
    return sort(circles, by = c -> dist(Point(0.5,0.5), c.center))
end

function sort_with_random(circles::Vector{Circle}, rng::AbstractRNG = Random.GLOBAL_RNG)
    return shuffle(rng, circles)
end

function sort_with_xcoordinate(circles::Vector{Circle})
    return sort(circles, by = c -> getx(c.center))
end

function sort_with_ycoordinate(circles::Vector{Circle})
    return sort(circles, by = c -> gety(c.center))
end

function sort_with_angles_from_origin(circles::Vector{Circle})
    return sort(circles, by = c -> atan(gety(c.center), getx(c.center)))
end

function sort_with_angles_from_center(circles::Vector{Circle})
    return sort(circles, by = c -> atan(gety(c.center) - 0.5, getx(c.center) - 0.5))
end
...
function main(argv)
    ...
    # sort circles
    @info "Sorting circles with mode: $sortmode"
    if sortmode == :origin
        circles = sort_with_origin(circles)
    elseif sortmode == :center
        circles = sort_with_center(circles)
    elseif sortmode == :random
        circles = sort_with_random(circles)
    elseif sortmode == :xcoordinate
        circles = sort_with_xcoordinate(circles)
    elseif sortmode == :ycoordinate
        circles = sort_with_ycoordinate(circles)
    elseif sortmode == :angles_from_origin
        circles = sort_with_angles_from_origin(circles)
    elseif sortmode == :angles_from_center
        circles = sort_with_angles_from_center(circles)
    else
        error("unknown sort mode: $sortmode")
    end

    # draw
    fig = plot_with_order(circles)
    savefig(fig, "sorted_circles_$(sortmode).png")
    ...
    @info "Nodes phi1: $(dag_size(res.varphi1))"
    @info "Nodes phi2: $(dag_size(res.varphi2))"
    @info "Peak Nodes: $(peak_node_count(M))"
    @info "Total Nodes: $(node_count(M))"
    @info "Memory in use (MB): $(memory_in_use(M) / 1024^2)"
