...
function readconfigfromJSON(filename)
    ...
    seed = config["seed"]
    samples = get(config, "samples", 1)
    return area, pdsradius, radius_domain, seed, samples
end

function make_output_filename(output_filename::AbstractString, i::Int, samples::Int)
    if samples <= 1
        return output_filename
    end
    width = max(3, length(string(samples)))
    base, ext = splitext(output_filename)
    return string(base, "-", lpad(i, width, '0'), ext)
end
...
function main(argv)
    ...
    area, pdsradius, radius_domain, seed, samples = readconfigfromJSON(filename)
    @info "Samples: $samples"

    for i in 1:samples
        rng = MersenneTwister(seed + i - 1)
        points = pds(pdsradius; area=area, rng=rng)
        @info "Generated $(length(points)) points (sample $i)"
        ...
        sample_output = make_output_filename(output_filename, i, samples)
        open(sample_output, "w") do io
            JSON.print(io, circles_json, 2)
        end
        @info "Wrote circles to $sample_output"
    end
end
